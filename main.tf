...
resource "aws_instance" "chef-server" {
  ...
  ami = var.ami_map["${var.ami_os}-${var.aws_region}"]
  count = 1
  ...
  connection {
    type = "ssh"
    host = self.public_ip
    user = var.ami_usermap[var.ami_os]
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo chef-solo --chef-license accept -j ~/.chef/dna.json -o 'recipe[chef-server::default],recipe[chef-server::addons]'",
    ]
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo chown -R ${var.ami_usermap[var.ami_os]} .chef",
    ]
  }
  ...
  # Copy back .chef files
  provisioner "local-exec" {
    command = "scp -r -o stricthostkeychecking=no -i ${var.instance_key["file"]} ${var.ami_usermap[var.ami_os]}@${self.public_ip}:.chef/* .chef/"
  }
...
connection {
host        = aws_instance.chef-server[0].public_ip
user        = var.ami_usermap[var.ami_os]
private_key = file(var.instance_key["file"])
}
...
node_name               = aws_instance.chef-server[0].tags.Name
...
server_url              = "https://${aws_instance.chef-server[0].tags.Name}/organizations/${var.chef_org["short"]}"
skip_install            = true
...
vars = {
user   = var.chef_user["username"]
pass   = base64sha256(aws_instance.chef-server[0].id)
user_p = ".chef/${var.chef_user["username"]}.pem"
fqdn   = aws_instance.chef-server[0].tags.Name
org    = var.chef_org["short"]
}
}
